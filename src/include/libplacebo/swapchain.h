...
// sister API to ra.h and follows the same convention w.r.t undefined behavior.

struct ra_swapchain {
    struct pl_context *ctx;   // the pl_context this swapchain was allocated from
    const struct ra *ra;      // the RA instance this swapchain belongs to
    const struct ra_sw *impl; // the underlying implementation
    void *priv;
};
...
void ra_swapchain_destroy(const struct ra_swapchain **sw);
...
// The struct used to hold the results of `ra_swapchain_start_frame`
struct ra_swapchain_frame {
    ...
    // the `ra_pass` that is rendering to this framebuffer.
    //
...
// lockstep with ra_swapchain_start_frame - there is no way to start multiple
...
// specifics depend on the mechanism used to create the ra_swapchain. (See the
...
// built-in latency. See `ra_swapchain_latency` for more information).
//
